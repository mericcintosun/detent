...
| High | A malformed record URL answers a real 404 with noindex, and React errors #419 and #418 stop on the record and not found pages. | Both `loading.tsx` boundaries streamed a 200 before `notFound()` ran, and the pending Suspense boundary raced hydration. The boundaries were removed, at the cost of no skeleton during a slow relay read. |
| High | The audit record reports the release the server actually performed: a keyless send says the policy was never installed and the local lock closed, and a live send says detached and revoked. | The console wrote "Policy revoked" even when the server reported nothing to revoke. |
| Medium | "Send edited plan" sends nothing when the edited amount equals the approved one, and each invalid amount gets its own message: empty, negative, comma separated, non numeric, or more than six decimals. | An equal value signed the approved plan and spent the lock, and every invalid input showed the six decimals message. |
| Medium | The permanent record opens in a new tab, and changing the corporate action under a lock asks first and writes a "Lock released" line with the reason. | Same tab navigation wiped the console session, and a lock was dropped without a trace. |
| Medium | Focus moves to the policy card after a lock, to the key banner after a send and to the error after a failure, and at 390 px the plan table keeps its status column in view with a sideways scroll hint. | Keyboard users lost their place, and row controls sat off screen on phones. |
| Medium | Colour contrast meets WCAG AA: serious axe violations went from 80 and 88 nodes to 0. The primary foreground is now ink, the muted foreground is 70 percent ink, and the ok and ring colours are darker, recorded as an IDENTITY.md amendment. | Light text on gold measured 2.82 to 1 and muted labels 4.11 to 4.43 to 1. |
| Low | A live ATS compliance refusal is labelled "Compliance contract hold" rather than "Sanctions screening hold", and the forced transfer accepts it as its subject. | ATS has no sanctions specific error, so the old label claimed something the chain never said. The seeded sanctions hold keeps its own label. |
| Low | The duplicated refusal text, the repeated record page sentence, the stale SEND marker and a header held count that changed with the selected action are fixed. | Inconsistencies found while browsing every state. |
| Note | The three end to end tests held with `test.fixme` for these bugs now run. | They were written first against the correct behaviour and waited for the fixes. |
