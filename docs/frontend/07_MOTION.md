...
| Refusal knock | `components/console/send-section.tsx`, `data-slot="send-knock"` | the wallet refuses | `x` keyframes 0, -6, 5, -3, 0 px over `duration.slow` 320 ms, `easing.standard`; two identical labels so each refusal plays | Motion | no movement; the oxide ring and the alert carry it |
