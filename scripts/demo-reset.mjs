...
if (holders === null) {
  console.error("demo:reset: fixtures/register.seed.json has no holders array.");
  process.exit(1);
}
...
if (failures.length > 0) {
  console.error("demo:reset: the register fixture is not valid, so the demo would not open cleanly.");
...
console.log(`  register       ${report.holderCount} holders on partition CLASS-A`);
console.log(`  compliance     ${report.heldCount} rows held, which is what turns them oxide red`);
console.log("  policies       none, the in memory vault starts empty in a fresh process");
console.log("  audit record   empty until the first lock");
console.log("");
console.log("Not reset, on purpose:");
console.log("  the on chain anchors. PlanAnchor has no reset entrypoint and a plan");
console.log("  hash is permanent, so a rehearsed run re-anchors nothing. lib/anchor.ts");
console.log("  reads planOf before it writes, reuses the record it finds, and the");
