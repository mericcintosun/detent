...
const COUPON_PLAN_HASH: Hex =
  "0xb43da729424748e5aaa28d8f13a1be304e9a455635c7c8d846726d148e849910";
...
const FORCED_PLAN_HASH: Hex =
  "0x6d8ca7cf2c92b1ae4db9cc039ff0be2e1803374ce11c0bb425c391b4c1c0a2bd";
...
describe("the coupon calldata", () => {
  ...
  it("is what the compiled policy pins, so the ALLOW rule is not self referential", () => {
    ...
    const prefix = conditions.find(
      (condition) =>
        condition.field === "data" && condition.operator === "starts_with",
    );
  ...
  it("is empty when every row is out of the plan", () => {
    ...
    expect(empty.blockers).toContain(
      "The plan is empty, nothing would be signed.",
    );
  });
});
...
describe("hold detection", () => {
  ...
  it("blocks the plan when a held row is forced back in", () => {
    ...
    expect(
      pushed.blockers.some((entry) => entry.includes("compliance module")),
    ).toBe(true);
